...
/**
 * \brief Declaration of DetectorCapsuleBox.
 */

#ifndef KCD_DETECTOR_CAPSULE_BOX_HH_
# define KCD_DETECTOR_CAPSULE_BOX_HH_
...
namespace hpp
{
  namespace geometry
  {
    namespace collision
    {
      class DetectorCapsuleBox : public CkcdDetector
      {
      public:
	/// \brief Create a new detector.
	static DetectorCapsuleBoxShPtr create ();

	/// \brief Create a copy of the detector.
	static DetectorCapsuleBoxShPtr
	...
	///	CkcdTestTreeBox::boundingBoxDispatchID().
	virtual bool canHandle (unsigned int leftID, unsigned int rightID) const;

      protected:
	/// \brief Initialize detector.
	ktStatus init (const DetectorCapsuleBoxWkPtr& weakPtr);
	...
	DetectorCapsuleBox (const DetectorCapsuleBox& detector);
      ...
      private:
	DetectorCapsuleBoxWkPtr weakPtr_;
      };
...
#endif //! KCD_DETECTOR_CAPSULE_BOX_HH_
